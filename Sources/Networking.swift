        ...
        if let token: String = User.current.oauth.accessToken{
            ...
            Alamofire
                ...
                .response { (request, response, data, error) in
                    if error != nil{
                        print(error?.localizedDescription)
                        self.getAllSpotDelegate?.getAllSpotsDidFail(error!)
                        return
                    }
                    
                    let  jsonData = JSON(data: data!)
                    print(jsonData)
                    self.getAllSpotDelegate?.getAllSpotsDidSucceed(jsonData)
            }
        }
    }
    
    class func loadSpotsForPriest() {
        if let token: String = User.current.oauth.accessToken{
            
            let parameters = ["access_token": token]
            let alamofireMethod: Alamofire.Method = .GET
            let link = Constants.priestSpotsLink_All;
